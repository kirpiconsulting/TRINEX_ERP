...
def run_table_sync():
    ...
    code, msg, data = rapor_calistir("D_ENT_01")
    if code != "200":
        print("API HATA:", msg)
        conn.close()
        return
    ...
    cur.execute("SELECT ek_alan_1 FROM depolar")
    ...
    for d in data:
        ...
        if ek in mevcutlar:
            ...
            cur.execute("""
                INSERT INTO depolar (ek_alan_1, kod, ad, aktif_mi, olusturma_tarihi, olusturan_kullanici)
                VALUES (?, ?, ?, ?, ?, 'ENTEGRASYON_01')
            """, ek, kod, ad, aktif, suan)

    for ek in mevcutlar:
        if ek not in keys:
            cur.execute("DELETE FROM depolar WHERE ek_alan_1=?", ek)
    ...
    conn.commit()
    conn.close()
    print("Depolar başarıyla güncellendi.")
...
if __name__ == "__main__":
    while True:
        run_table_sync()
        cfg = oku_json()
        dakika = int(cfg.get("periyot_dakika_depolar", 30))
