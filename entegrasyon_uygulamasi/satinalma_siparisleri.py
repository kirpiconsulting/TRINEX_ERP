...
def run_table_sync():
    ...
    code, msg, data = rapor_calistir("D_ENT_09")
    if code != "200":
        print("API HATA:", msg)
        conn.close()
        return
    ...
    cur.execute("SELECT ek_alan_1 FROM satinalma_siparisleri")
    ...
    for d in data:
        ...
        if ek in mevcutlar:
            ...
            cur.execute("""
                INSERT INTO satinalma_siparisleri (ek_alan_1, evrak_no, tarih, cari_kod, olusturma_tarihi, olusturan_kullanici)
                VALUES (?, ?, ?, ?, ?, 'ENTEGRASYON_01')
            """, ek, fisno, tarih, cari, suan)

    for ek in mevcutlar:
        if ek not in keys:
            cur.execute("DELETE FROM satinalma_siparisleri WHERE ek_alan_1=?", ek)

    cur.execute("""
        UPDATE entegrasyon_tanimlari 
        SET deger_1=? 
        WHERE kod='satinalma_siparisleri'
    """, datetime.now())

    conn.commit()
    conn.close()
    print("Satınalma Siparişleri başarıyla güncellendi.")
...
if __name__ == "__main__":
    while True:
        run_table_sync()
        cfg = oku_json()
        dakika = int(cfg.get("periyot_dakika_satinalma_siparisleri", 30))
