...
def run_table_sync():
    ...
    code, msg, data = rapor_calistir("D_ENT_05")
    if code != "200":
        print("API HATA:", msg)
        conn.close()
        return
    ...
    cur.execute("SELECT ek_alan_1 FROM tezgahlar")
    ...
    for d in data:
        ...
        if ek in mevcutlar:
            cur.execute("""
                UPDATE tezgahlar SET 
                    kod=?, ad=?, aktif_mi=?, guncelleme_tarihi=?, guncelleyen_kullanici='ENTEGRASYON_01'
                WHERE ek_alan_1=?
            """, kod, ad, aktif, suan, ek)
        else:
            cur.execute("""
                INSERT INTO tezgahlar (ek_alan_1, kod, ad, aktif_mi, olusturma_tarihi, olusturan_kullanici)
                VALUES (?, ?, ?, ?, ?, 'ENTEGRASYON_01')
            """, ek, kod, ad, aktif, suan)

    for ek in mevcutlar:
        if ek not in keys:
            cur.execute("DELETE FROM tezgahlar WHERE ek_alan_1=?", ek)
    ...
    conn.commit()
    conn.close()
    print("Tezgahlar başarıyla güncellendi.")
...
if __name__ == "__main__":
    while True:
        run_table_sync()
        cfg = oku_json()
        dakika = int(cfg.get("periyot_dakika_tezgahlar", 30))
