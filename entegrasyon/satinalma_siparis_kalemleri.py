...
def run_table_sync():
    ...
    code, msg, data = rapor_calistir("D_ENT_10")
    if code != "200":
        print("API HATA:", msg)
        conn.close()
        return
    ...
    cur.execute("SELECT kalem_kodu FROM satinalma_siparis_kalemleri")
    ...
    for d in data:
        ...
        if kalem_kodu in mevcutlar:
            cur.execute("""
                UPDATE satinalma_siparis_kalemleri SET 
            ...
            cur.execute("""
                INSERT INTO satinalma_siparis_kalemleri (kalem_kodu, evrak_no, stok_kod, miktar,stok_birim,teslim_tarihi, olusturma_tarihi,sira_numarasi, olusturan_kullanici)
    ...
    for kalem_kodu in mevcutlar:
        if kalem_kodu not in keys:
            cur.execute("DELETE FROM satinalma_siparis_kalemleri WHERE kalem_kodu=?", kalem_kodu)

    cur.execute("""
        UPDATE entegrasyon_tanimlari 
        SET deger_1=? 
        WHERE kod='satinalma_siparis_kalemleri'
    """, datetime.now())

    conn.commit()
    conn.close()
    print("Satınalma Sipariş Kalemleri başarıyla güncellendi.")
...
if __name__ == "__main__":
    while True:
        run_table_sync()
        cfg = oku_json()
        dakika = int(cfg.get("periyot_dakika_satinalma_siparis_kalemleri", 30))
