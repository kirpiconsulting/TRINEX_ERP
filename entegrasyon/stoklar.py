...
def run_table_sync():
    ...
    for d in data:
        ...
        if ek in mevcutlar:
            cur.execute("""
                UPDATE stoklar SET 
                    kod=?, ad=?, aktif_mi=?, guncelleme_tarihi=?, ana_birim=?, ek_alan_2=?, ek_alan_3=?, guncelleyen_kullanici='ENTEGRASYON_01'
                WHERE ek_alan_1=?
            """, kod, ad, aktif, suan,anabirim,ek2,ek3, ek)
        else:
            cur.execute("""
                INSERT INTO stoklar (ek_alan_1, kod, ad, aktif_mi, olusturma_tarihi, olusturan_kullanici, ana_birim, ek_alan_2, ek_alan_3)
                VALUES (?, ?, ?, ?, ?, 'ENTEGRASYON_01',?, ?, ?)
            """, ek, kod, ad, aktif, suan, anabirim, ek2, ek3)
